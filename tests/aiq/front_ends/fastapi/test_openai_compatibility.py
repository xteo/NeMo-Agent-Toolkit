...
def test_aiq_chat_response_chunk_create_streaming_chunk():
    ...
    chunk = AIQChatResponseChunk.create_streaming_chunk(
        content="Hello",
        role="assistant"
    )
...
async def test_legacy_vs_openai_compatible_mode_endpoints(openai_api_compatible: bool):
    ...
    async with _build_client(config) as client:
        ...
        if openai_api_compatible:
            ...
            has_content = any(
                (chunk.choices[0].message and chunk.choices[0].message.content) or
            ...
            async with aconnect_sse(
                ...
                "POST",
                f"{base_path}/stream",
                json={"messages": [{"content": "World", "role": "user"}]}
            ) as event_source:
                async for sse in event_source.aiter_sse():
                    if sse.data != "[DONE]":
                        chunk = AIQChatResponseChunk.model_validate(sse.json())
            ...
            has_message_content = any(
                chunk.choices[0].message and chunk.choices[0].message.content 
                for chunk in response_chunks
            )
...
async def test_legacy_mode_backward_compatibility():
    ...
    async with _build_client(config) as client:
        ...
        assert chunk.choices[0].delta is None or (
            chunk.choices[0].delta.content is None and chunk.choices[0].delta.role is None
        )
